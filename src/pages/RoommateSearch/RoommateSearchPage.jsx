...
export default function RoommateSearchPage() {
  ...
  const filteredRoommates = roommates.filter((roommate) => {
    ...
    const matchesBudgetMin =
      !filters.budgetMin || roommateTotalBudget >= parseInt(filters.budgetMin);
    const matchesBudgetMax =
      !filters.budgetMax || roommateTotalBudget <= parseInt(filters.budgetMax);
  ...
  if (filters.newest) {
    ...
    sortedRoommates.sort(
      (a, b) => (dateMap[a.createdAt] || 0) - (dateMap[b.createdAt] || 0)
    );
  }
  ...
  if (filters.sortByBudget === "asc") {
    sortedRoommates.sort(
      (a, b) =>
        a.rentBudget + a.depositBudget - (b.rentBudget + b.depositBudget)
    );
  } else if (filters.sortByBudget === "desc") {
    sortedRoommates.sort(
      (a, b) =>
        b.rentBudget + b.depositBudget - (a.rentBudget + a.depositBudget)
    );
  }
              ...
              <div className="p-4 flex flex-col h-full">
                <div className="flex flex-col items-center text-center mb-4">
                  ...
                  <p className="text-gray-500 text-sm">
                    {roommate.gender === "female" ? "زن" : "مرد"}، {roommate.age}{" "}
                    ساله
                  </p>
